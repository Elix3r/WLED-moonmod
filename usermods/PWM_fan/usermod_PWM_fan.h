...
private:
  ...
  // MQTT related
  String mqttTopicBase = "wled/" + String(networkHostname());
  ...
  void publishMQTT(const char* subtopic, uint16_t value) {
    if (mqtt != nullptr && WLED_MQTT_CONNECTED) {
      String topic = mqttTopicBase + "/" + String(subtopic);
      mqtt->publish(topic.c_str(), 0, false, String(value).c_str());
    }
  }
